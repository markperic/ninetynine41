...
/**
 * Module 66 — Hero, Image Stream
 * Two rails of image cards ride from far behind the screen toward the
 * viewer on pure CSS 3D perspective — no JS animation loop, the whole
 * corridor is a pair of generated @keyframes traced from the geometry
 * below. Adapted from 21st.dev's Image Stream Hero: shadcn CSS-variable
 * classes (`border-border`, `bg-background`, `text-muted-foreground`) swapped
 * for this repo's zinc palette since only `--background`/`--foreground` are
 * defined here, the source's Unsplash-adjacent CDN demo images swapped for
 * Pexels photos, and the copy swapped for this catalog's voice. Everything
 * inside the corridor comment block below is copied verbatim from the
 * source — it's dense, deliberate geometry math, not boilerplate to
 * simplify. Title effect: B. The corridor itself is a custom CSS-keyframe
 * animation, not part of the A–J catalog — same as module 65's accordion.
 */
...
export default function HeroImageStream() {
  return (
    <section className="relative h-[640px] w-full overflow-hidden bg-zinc-950">
      <ImageStreamHero images={IMAGES} className="h-full w-full">
        <div className="relative z-10 flex h-full flex-col items-center justify-between px-6 py-16 text-center">
          <Reveal effect="B" as="h1" className="text-balance text-4xl font-semibold tracking-tight text-white sm:text-5xl">
            Your work,
            <br />
            front and centre.
          </Reveal>
          <div>
            <p className="mx-auto max-w-md text-balance text-sm text-zinc-400">
              A hero that leads with the images instead of describing them. Drop in your own and the corridor rebuilds around them.
            </p>
            <a
              href="#"
              className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-medium text-zinc-950 transition-colors hover:bg-zinc-200"
            >
              See how it works
